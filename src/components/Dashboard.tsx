...
const StatCard = ({ title, value, icon, trend, accent }: StatCardProps) => (
  <div className={`rounded-xl md:rounded-2xl p-3 md:p-5 transition-all ${accent ? 'bg-gradient-to-br from-indigo-500 to-purple-600 text-white' : 'bg-white border border-gray-100'}`}>
    <div className="flex items-center gap-2 md:gap-3">
      <div className={`p-2 rounded-lg md:rounded-xl shrink-0 ${accent ? 'bg-white/20' : 'bg-indigo-50'}`}>
        {icon}
      </div>
      <div className="min-w-0 flex-1">
        <p className={`text-lg md:text-2xl font-bold truncate ${accent ? 'text-white' : 'text-gray-900'}`}>{value}</p>
        <p className={`text-[10px] md:text-xs ${accent ? 'text-white/70' : 'text-gray-500'}`}>{title}</p>
      </div>
      {trend !== undefined && (
        <div className={`flex items-center gap-0.5 text-[10px] md:text-xs font-medium shrink-0 ${trend >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
